...
const DeadlineRanking = () => {
    ...
    const fetchDeadlineRankings = async () => {
        try {
            setLoading(true);
            ...
            console.error("Error fetching deadline rankings:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const getDeadlineStatusColor = (missed, total) => {
        if (missed === 0 && total > 0) return "text-[var(--success)]";
        if (missed <= Math.ceil(total / 2)) return "text-[var(--warning)]";
        return "text-[var(--danger)]";
    };

    const getStatusBadge = (missed, total) => {
        if (missed === 0 && total > 0) {
            return { label: "Perfect", color: "bg-[var(--success)]/10 text-[var(--success)] border-[var(--success)]/20" };
        }
        if (missed <= 1) {
            return { label: "Good", color: "bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border-[var(--accent-primary)]/20" };
        }
        if (missed <= Math.ceil(total / 2)) {
            return { label: "Needs Improvement", color: "bg-[var(--warning)]/10 text-[var(--warning)] border-[var(--warning)]/20" };
        }
        return { label: "Critical", color: "bg-[var(--danger)]/10 text-[var(--danger)] border-[var(--danger)]/20" };
    };
            ...
            <div className="p-4 md:p-6">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-lg font-medium text-sm transition-all ${filters.filterBy === "missed"
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg font-medium text-sm transition-all flex items-center gap-1 ${filters.sortOrder === "desc"
                                            ? "ui-btn ui-btn-primary"
                                            : "ui-btn"
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg font-medium text-sm transition-all flex items-center gap-1 ${filters.sortOrder === "asc"
                                            ? "ui-btn ui-btn-primary"
                                            : "ui-btn"
                                        }`}
                                >
                ...
                <div className="mb-4 flex items-center gap-2 flex-wrap">
                    ...
                    <span className="px-2.5 py-1 bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] text-xs font-medium rounded-full border border-[var(--accent-primary)]/20">
                        Missed Deadlines - {filters.sortOrder === "desc" ? "High to Low" : "Low to High"}
                    </span>
                    {searchTerm && (
                ...
                {loading ? (
                    ...
                    <div className="ui-card p-12 text-center">
                        ...
                        <p className="text-[var(--text-muted)]">
                            No deadline data available for the selected criteria.
                        </p>
                    </div>
                ) : (
                    <div className="space-y-3">
                                        ...
                                        <div className="flex items-center gap-3 flex-1">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1 flex-wrap">
                                                    ...
                                                    <span
                                                        className={`px-2 py-0.5 text-xs font-semibold rounded-full border ${employee.user?.isActive
                                        ...
                                        <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto justify-between md:justify-end">
                                            <div className="text-center">
                                                <div className="flex items-center gap-1 justify-center">
                                                    ...
                                                    <span className={`text-xl font-bold ${getDeadlineStatusColor(employee.missedDeadlines, employee.tasksWithDeadline)}`}>
                                                        {employee.missedDeadlines || 0}
                                                    </span>
                                                </div>
                                                <p className="text-xs text-[var(--text-muted)] mt-1">Missed</p>
                                            </div>
                                            ...
                                            <div className="text-center">
                                                ...
                                                <p className="text-xs text-[var(--text-muted)] mt-1">Total Deadlines</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-all ${pagination.hasPrevPage
                                            ? "ui-btn"
                                        ...
                                        if (
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-10 h-10 rounded-lg font-medium text-sm transition-all ${pagination.currentPage === pageNum
                                                            ? "ui-btn ui-btn-primary"
                                                            : "ui-btn"
                                                        }`}
                                                >
                                ...
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-all ${pagination.hasNextPage
                                            ? "ui-btn"
                                            : "bg-[var(--bg-secondary)] text-[var(--text-muted)] cursor-not-allowed border border-[var(--border-color)]"
                                        }`}
                                >
